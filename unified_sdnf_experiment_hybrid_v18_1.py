...
SDNF v18.1: reviewer-grade, single-file experiment harness for
"Semantic Data Normal Forms: Extending Normalization Theory to Vector Embedding Spaces".

v18.1 builds on v17's proven infrastructure and applies the KeyFix remediation:
...
__version__ = "18.1.0"
DEFAULT_SEED = 42
DEFAULT_OUTPUT_DIR = "output_v18_1"
...
class OutputBudgetWriter:
    """All v18.1 output writes pass through this budget enforcer."""
...
def evaluate_pair(a: SchemaAttribute, b: SchemaAttribute, args: argparse.Namespace, track: str, negative_pairs: Set[Pair], true_pairs: Optional[Set[Pair]]) -> MergeDecision:
    ...
    same_canon = a.canonical_key == b.canonical_key  # v18.1: compute early for semantic_vetoes
    ...
    cross_global = same_family and a.semantic_family in GLOBAL_CROSS_RAIL_FAMILIES  # v18.1: auto-allow
    ...
    # v18.1 FN-FIX: CMNF-matrix canonical-safe acceptance
    ...
    # v18.1 FN-FIX: Same canonical + same family at tau_aanf
    ...
    # v18.1 FN-FIX: Alias-hit promotion
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run SDNF v18.1 reviewer-grade experiment harness.")
...
# ═══════════════════════════════════════════════════════════════════════════════
# v18.1 PATCH: Schema-truth evaluation + Consolidated reviewer helpers
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_consolidated_reviewer_rows(
    ...
    decision pair. Returns rows matching fn_tp_human_review_v18_1.csv format
...
def main() -> None:
    ...
    # ═══ v18.1 PATCH: Schema-truth evaluation ═══
    ...
    # ═══ v18.1 PATCH: Schema-truth report for console/out_audit ═══
    ...
    if "out_audit_v18_1.txt" in allowed:
        writer.write_text("out_audit_v18_1.txt", out_text)

    if "run_manifest_v18_1.json" in allowed:
        writer.write_json("run_manifest_v18_1.json", manifest)

    if "summary_audit_v18_1.json" in allowed:
        writer.write_json("summary_audit_v18_1.json", summary)

    if "srs_evolved_schema_v18_1.compact.json" in allowed:
        writer.write_json("srs_evolved_schema_v18_1.compact.json", {k: serialize_node(v) for k, v in sorted(nodes.items())})

    if "schema_ingestion_audit_v18_1.csv" in allowed:
        writer.write_csv("schema_ingestion_audit_v18_1.csv", schema_audit + [a for a in payload_audit])

    if "field_evidence_audit_v18_1.csv" in allowed:
        writer.write_csv("field_evidence_audit_v18_1.csv", [asdict(fe) for fe in field_evidence_rows])

    if "schema_deltas_audit_v18_1.csv" in allowed:
        writer.write_csv("schema_deltas_audit_v18_1.csv", schema_deltas)

    if "decisions_audit_v18_1.csv" in allowed:
        dec_rows = []
        for d in decisions:
            dec_rows.append({
        ...
        writer.write_csv("decisions_audit_v18_1.csv", dec_rows)

    if "alias_evaluation_audit_v18_1.csv" in allowed:
        ...
        # v18.1 PATCH: Add schema-truth metrics with prefix
        ...
        # v18.1 PATCH: Add lexicon-quality metrics with prefix
        if alias_metrics.get("measurable"):
        ...
        writer.write_csv("alias_evaluation_audit_v18_1.csv", alias_rows)

    if "payload_compliance_audit_v18_1.csv" in allowed:
        writer.write_csv("payload_compliance_audit_v18_1.csv", [asdict(pc) for pc in payload_compliance])

    if "normal_forms_and_claims_audit_v18_1.csv" in allowed:
        writer.write_csv("normal_forms_and_claims_audit_v18_1.csv", claims)

    if "scale_timing_drift_audit_v18_1.csv" in allowed:
        writer.write_csv("scale_timing_drift_audit_v18_1.csv", scale_rows)

    if "review_queue_audit_v18_1.csv" in allowed:
        consolidated_fields = [
        ...
        writer.write_csv("review_queue_audit_v18_1.csv", consolidated_reviewer_rows, fields=consolidated_fields)


    if "sdnf_debug_bundle_v18_1.zip" in allowed:
        writer.write_zip("sdnf_debug_bundle_v18_1.zip", [f for f in writer.written if f != "sdnf_debug_bundle_v18_1.zip"])
